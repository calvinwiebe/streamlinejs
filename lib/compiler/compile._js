...
exports.compileFile = function(_, path, options) {
	...
	if (options.noWrite) {
		...
		try {
			fs.writeFile(dstName, sources.transformed, 'utf8', ~_);
		} catch (ex) {}
		if (options.sourceMap) {
			if (options.prevMap) {
				sources.sourceMap.applySourceMap(options.prevMap, options.sourceName);
			}
			try {
				if (options.verbose) console.log("streamline: creating: " + mapFile);
				fs.writeFile(mapFile, sources.sourceMap.toString(), 'utf8', ~_);
			} catch (ex) {}
		}
	}
}
